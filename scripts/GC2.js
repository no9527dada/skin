...
exports.jidianchi4 = jidianchi4;
  lib.setBuildingSimple(extendContent(Wall, "wu-di-qiang", {}), Wall.WallBuild, {//无敌墙
    ...
    return new JavaAdapter(GenericCrafter.GenericCrafterBuild, {
        ...
        remove() {
            ...
            draw() {
                ...
                Draw.blend();
                Draw.color();
                Draw.rect(Core.atlas.find("creator-weichenshoujiqi-1"), this.x, this.y, 45 - Time.time * 1);  
            },
    }, weichenshoujiqi);
});
...
weichenshoujiqi.craftTime = 1080000;
weichenshoujiqi.updateEffect = Fx.lightningCharge;
weichenshoujiqi.updateEffectChance = 0.2;
...
wupinyuan.buildCostMultiplier = 2000;//建造时间倍
...
lib.addToResearch(block, { parent: Blocks.commandCenter.name, });//科技树//commandCenter
...
exports.block = block;
//----------------------------------------------------
const display = extend(LogicDisplay, "display", {});//显示器
display.health = 500;
display.size = 16;
display.hasPower = true
display.consumes.power(3);
display.displaySize = 480;
display.requirements = ItemStack.with(
    Items.copper, 1000,
    Items.lead, 1200,
    Items.metaglass, 120,
    shiying, 200,
...
display.buildVisibility = BuildVisibility.shown;
display.category = Category.logic;
lib.addToResearch(display, { parent: Blocks.largeLogicDisplay.name, });
